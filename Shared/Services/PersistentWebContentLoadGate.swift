...
final class PersistentWebContentLoadGate {
    ...
    func load(
        ...
        guard !PersistentJavaScriptLibrary.requiredDependencies(in: html).isEmpty else {
            cancel()
            return false
        }
        ...
        task = Task { [weak self] in
            do {
                let resolved = try await PersistentJavaScriptLibrary.resolve(html, cache: cache, allowsDownloads: allowsDownloads)
