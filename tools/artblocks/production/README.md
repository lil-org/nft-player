...
The complete original 519-project review corpus and the pre-promotion production catalog are retained in `../archive/pass-5`. Exact exports, approved manifests, and the cumulative 109-project static list remain in `../reviews`. The cumulative 736-project rejection ledger is `../rejected.json`. Discovery and sample selection consult both deferred and rejected identities. Existing sample folders are not deleted by this workflow.
