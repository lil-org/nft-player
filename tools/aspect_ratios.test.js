...
test("chooses a normalized default by frequency and first appearance", () => {
  ...
  assert.deepEqual(encodeAspectRatioMetadata(result, decodeAspectRatioMetadata(result)), result);
});
...
test("decodes collection defaults, exceptions, token-only values, and missing values", () => {
  ...
  assert.equal(decodeAspectRatioMetadata({ aspectRatio: null, items: [{ id: "a", aspectRatio: null }] }), null);
});
...
test("rejects malformed collection and token ratios", () => {
  for (const aspectRatio of [[], [1], [1, 1, 1], [0, 1], [-1, 1], [1.5, 1], ["1", 1], "1:1", {}]) {
    assert.throws(() => decodeAspectRatioMetadata({ aspectRatio, items: [{ id: "a" }] }));
    assert.throws(() => decodeAspectRatioMetadata({ aspectRatio: [1, 1], items: [{ id: "a", aspectRatio }] }));
...
async function preserveFixture(t, existing, next) {
  ...
  return preserveAspectRatioMetadataFromFile(filePath, next);
}
...
test("reselects the most common default after removal", async (t) => {
  ...
  assert.deepEqual(result.payload, { aspectRatio: [4, 3], items: [{ id: "b" }] });
});

test("omits all metadata when a new token has no preserved ratio", async (t) => {
  const result = await preserveFixture(t, {
    aspectRatio: [16, 9], items: [{ id: "a" }],
  }, { aspectRatio: [1, 1], items: [{ id: "a", aspectRatio: [4, 3] }, { id: "new" }] });
...
test("missing source leaves the next payload without stale metadata", async (t) => {
  const result = await preserveFixture(t, null, {
    aspectRatio: [1, 1], items: [{ id: "a", aspectRatio: [4, 3] }],
  });
